...
HEADER_ONLY_STRUCTS = {"RawFrameMetadata"}
...
def ts_type(rust_type: str, *, option_as_optional: bool = False) -> str:
	...
	if rust_type == "String" or rust_type == "&str":
		return "string"
	if rust_type in {"WindowPreset", "FileSummary", "TagNode", "TagValue"}:
		return rust_type
...
def raw_frame_header_key(const_name: str) -> str:
	suffix = const_name.removeprefix("RAW_FRAME_HEADER_").lower()
	parts = suffix.split("_")
	return parts[0] + "".join(part.capitalize() for part in parts[1:])


def render_raw_frame_headers(source: str) -> str:
	matches = re.findall(r'pub const (RAW_FRAME_HEADER_[A-Z0-9_]+): &str =\s*"([^"]+)";', source)
	if not matches:
		raise ValueError("raw frame header constants not found")

	lines = ["export const RAW_FRAME_HEADERS = {"]
	for name, value in matches:
		lines.append(f'\t{raw_frame_header_key(name)}: "{value}",')
...
def validate_struct_coverage(source: str) -> None:
	serializable_structs = {
		...
		for match in re.finditer(
			r"#\[derive\([^\)]*Serialize[^\)]*\)\]\s*pub struct ([A-Za-z0-9_]+)",
			source,
			re.MULTILINE,
		)
	}
	classified = set(STRUCTS) | HEADER_ONLY_STRUCTS
	missing = sorted(serializable_structs - classified)
	if missing:
		raise ValueError(
			"serializable Rust structs are not generated or classified as header-only: "
			+ ", ".join(missing)
		)
